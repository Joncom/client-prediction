...
.defines(function(){
    ...
    InputAndStateList = ig.Class.extend({ // static class InputAndStateList {
        ...
        correct: function(currentState, serverState) { // TimedState correct(TimedState currentState, TimedState serverState) {
            if (serverState !== null) { // if (serverState != null) {
                ...
                if (!isOldestWithinThresholdTo(serverState.state)) {
                    System.out.println("perform correction " + this.list.get(0).timedState + " != " + serverState); // System.out.println("perform correction " + list.get(0).timedState + " != " + serverState);
        ...
        removeBefore: function(time) { // private void removeBefore(long time) {
            while (this.list.size() > 0 && this.list.get(0).timedState.time < time) { // while (list.size() > 0 && list.get(0).timedState.time < time) {
        ...
        isOldestWithinThresholdTo: function(state) { // private boolean isOldestWithinThresholdTo(State state) {
            return (this.list.size() > 0 && this.list.get(0).timedState.state.distance(state) <= TRESHOLD); // return (list.size() > 0 && list.get(0).timedState.state.distance(state) <= TRESHOLD);
        },
