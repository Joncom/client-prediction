    ...
    InterpolatedState = ig.Class.extend({ // static class InterpolatedState {
        ...
        update: function(now, timedState) { // void update(long now, TimedState timedState) {
            ...
            if (timedStateList.size() >= 2) {
                this.currentTime = Math.min(timedStateList.get(timedStateList.size() - 1).time, this.currentTime); // currentTime = Math.min(timedStateList.get(timedStateList.size() - 1).time, currentTime);
                this.currentTime = Math.max(timedStateList.get(timedStateList.size() - 1).time - 500, this.currentTime); // currentTime = Math.max(timedStateList.get(timedStateList.size() - 1).time - 500, currentTime);

                while (timedStateList.size() >= 2 && timedStateList.get(1).time < this.currentTime) { // while (timedStateList.size() >= 2 && timedStateList.get(1).time < currentTime) {
