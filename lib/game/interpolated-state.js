    ...
    InterpolatedState = ig.Class.extend({ // static class InterpolatedState {
        ...
        update: function(now, timedState) { // void update(long now, TimedState timedState) {
            ...
            if (timedState !== null) { // if (timedState != null) {
                ...
                this.prevState = newestState;
                prevStateTime = newestStateTime;
                newestState = timedState;
                newestStateTime = now;

                if (this.prevState.time !== 0 && newestState.time !== 0) { // if (prevState.time != 0 && newestState.time != 0) {
                    if (currentTime === 0) { // if (currentTime == 0) {
                        this.currentTime = newestState.time; // currentTime = newestState.time;
                    }
                    if (Math.abs(newestState.time - this.prevState.time) < 1000) { // if (Math.abs(newestState.time - prevState.time) < 1000) {
                        timeBetweenPackets = newestState.time - this.prevState.time; // float timeBetweenPackets = newestState.time - prevState.time;
                        ...
                        sendDelta = newestState.time - this.prevState.time; // long sendDelta = newestState.time - prevState.time;
                        ...
                        currentTimeBehindNewestState = newestState.time - this.currentTime; // long currentTimeBehindNewestState = newestState.time - currentTime;
                        targetTimeBehindNewestState = Math.parseInt((averageTimeBetweenPackets + this.averageJitter * 2), 10); // long targetTimeBehindNewestState = (long) (averageTimeBetweenPackets + averageJitter * 2);
            ...
            if (this.timedStateList.length >= 2) { // if (timedStateList.size() >= 2) {
                this.currentTime = Math.min(this.timedStateList.get(this.timedStateList.size() - 1).time, this.currentTime); // currentTime = Math.min(timedStateList.get(timedStateList.size() - 1).time, currentTime);
                this.currentTime = Math.max(this.timedStateList.get(this.timedStateList.size() - 1).time - 500, this.currentTime); // currentTime = Math.max(timedStateList.get(timedStateList.size() - 1).time - 500, currentTime);

                while (this.timedStateList.size() >= 2 && this.timedStateList.get(1).time < this.currentTime) { // while (timedStateList.size() >= 2 && timedStateList.get(1).time < currentTime) {
                    this.timedStateList.remove(0); // timedStateList.remove(0);
                }
                s1 = this.timedStateList.get(0); // TimedState s1 = timedStateList.get(0);
                s2 = this.timedStateList.get(1); // TimedState s2 = timedStateList.get(1);
                t = (this.currentTime - s1.time) / Math.parseFloat(s2.time - s1.time); // float t = (currentTime - s1.time) / (float) (s2.time - s1.time);
