...
.defines(function(){
    ...
    InterpolatedState = ig.Class.extend({ // static class InterpolatedState {
        ...
        update: function(now, timedState) { // void update(long now, TimedState timedState) {
            ...
            if (timedState !== null) { // if (timedState != null) {
                ...
                if (this.prevState.time !== 0 && this.newestState.time !== 0) { // if (prevState.time != 0 && newestState.time != 0) {
                    ...
                    if (Math.abs(this.newestState.time - this.prevState.time) < 1000) { // if (Math.abs(newestState.time - prevState.time) < 1000) {
                        ...
                        var jitter = recieveDelta - sendDelta; // long jitter = recieveDelta - sendDelta;
            ...
            var correction = 0; // long correction = 0;
            ...
            if (this.timedStateList.length >= 2) { // if (timedStateList.size() >= 2) {
                ...
                var s1 = this.timedStateList[0]; // TimedState s1 = timedStateList.get(0);
                var s2 = this.timedStateList[1]; // TimedState s2 = timedStateList.get(1);
                var t = (this.currentTime - s1.time) / parseFloat(s2.time - s1.time); // float t = (currentTime - s1.time) / (float) (s2.time - s1.time);
                this.currentState = new TimedState(this.currentTime, new State( // currentState = new TimedState(currentTime, new State(
