...
.defines(function(){
    ...
    InterpolatedState = ig.Class.extend({ // static class InterpolatedState {
        ...
        update: function(now, timedState) { // void update(long now, TimedState timedState) {
            delta = now - this.prevTime; // long delta = now - prevTime;
            prevTime = now;

            if (timedState !== null) { // if (timedState != null) {
                this.timedStateList.add(timedState); // timedStateList.add(timedState);
            ...
            if (this.timedStateList.size() >= 2) { // if (timedStateList.size() >= 2) {
