    ...
    InterpolatedState = ig.Class.extend({ // static class InterpolatedState {
        ...
        update: function(now, timedState) { // void update(long now, TimedState timedState) {
            ...
            if (this.timedStateList.length >= 2) { // if (timedStateList.size() >= 2) {
                this.currentTime = Math.min(this.timedStateList[this.timedStateList.size() - 1].time, this.currentTime); // currentTime = Math.min(timedStateList.get(timedStateList.size() - 1).time, currentTime);
                this.currentTime = Math.max(this.timedStateList[this.timedStateList.size() - 1].time - 500, this.currentTime); // currentTime = Math.max(timedStateList.get(timedStateList.size() - 1).time - 500, currentTime);
