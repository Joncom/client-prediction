    ...
    */
    ...
    renderer = ig.Class.extend({ // static JComponent renderer = new JComponent() {
        ...
        paintComponent: function(g) { // public void paintComponent(Graphics g) {
            ...
            this.drawPlayer(g, server.currentState.state, 'BLUE'); // drawPlayer(g, server.currentState.state, Color.BLUE.darker());
            this.drawPlayer(g, client.interpolatedState.newestState.state, 'RED'); // drawPlayer(g, client.interpolatedState.newestState.state, Color.RED.darker());
            this.drawPlayer(g, client.interpolatedState.currentState.state, 'GREEN'); // drawPlayer(g, client.interpolatedState.currentState.state, Color.GREEN.darker());
            this.drawPlayer(g, client.predictedState.currentState.state, 'BLACK'); // drawPlayer(g, client.predictedState.currentState.state, Color.BLACK);
